...
/*							*/
/*	@(#) uugetty.c: version 23.2 created on 3/13/91 at 10:23:29	*/
/*							*/
/*	Copyright (c) 1990 by Arix Corporation		*/
/*	All Rights Reserved				*/
/*							*/
#ident	"@(#)uugetty.c	23.2	3/13/91 Copyright (c) 1990 by Arix Corporation"
/*							*/
...
/*									*/
/*	Usage:								*/
/*	uugetty [-h] [-t time] line [speed_label] [terminal] [line_disc]*/
/*		[[-S] -P [-F] [-s] strmod [-f] [-s] strmod ...]		*/
/*									*/
...
/*	"speed_label" is a pointer into the "/etc/gettydefs"		*/
/*			where the definition for the speeds and		*/
/*			other associated flags are to be found.		*/
/*			If omitted then the first entry in the		*/
/*			"/etc/gettydefs" file is used.			*/
/*	"terminal" is the name of the terminal type (optional).		*/
/*	"line_disc" is the name of the line discipline (optional).	*/
/*									*/
/*	-P tells getty that everything else on the command line is	*/
/*	   name(s) of stream modules to be pushed.			*/
/*	-S tells getty to save current terminal setting and restore it	*/
/*	   after ALL of the specified modules have been pushed.		*/
/*	-F if specified in front of a particular stream module, will	*/
/*	   force its push even if the module has already been pushed.	*/
/*	   This option has effect until '-f' which turns it off.	*/
/*	-f if specified will turn off '-F' effect.			*/
/*	-s if specified in front of a particular stream module, will	*/
/*	   save current terminal settings before pushing the module	*/
/*	   and will restore the settings after the push is done.	*/
/*	   Any occurance of this argument will turn OFF	the -S		*/
/*	   argument.  Also, -s implies -P, so -P can be omitted	if	*/
/*	   the first stream module needs -s, otherwise -P is needed.	*/
/*	   NOTE:  If '-s' is found, the next word is taken to be a	*/
/*		  module name, so do not put anything between them.	*/
/*									*/
/*		 The modules are pushed in the order supplied on the	*/
/*		 cmd line.  The module name must have spelling and	*/
/*		 capitalization exactly	as the corresponding stream	*/
/*		 module name as defined in the kernel.			*/
/*									*/
...
/*	kd0	add timer support to uugetty				*/

/* dummies for using uucp .o routines */

#define	STREAMS	1		/* enable stream modules processing	*/
...
#include	<sys/poll.h>
#include	<fcntl.h>
void		wait_for_connect_notify();	/* Poll on msg from driver */
...
char	*checkgdfile;		/* Name of gettydefs file during
				 * check mode.
				 */
struct	gctio		i_gctio, f_gctio;	/*kd0*/
 
#ifdef STREAMS

#define	STRFLG_SAVE	0x01	/* save tty state before a push		*/
#define	STRFLG_FORCE	0x02	/* force push of already pushed module	*/

typedef	struct	strlisttab	/* mu1					*/
	{
		char	*name;	/* module name				*/
		int	flags;	/* see above defines for definitions	*/
	} strlisttab_t;

strlisttab_t	strlisttab[64];	/* be liberal and don't check bounds	*/
 
int		strsaveall;	/* save tty settings for all modules	*/
char		*defstrname = "ldterm"; /* The default Stream module to push*/

#endif	/* STREAMS */

int	wait_read = FALSE;	/* wait for read before output "login" */
char *debugfile = "/tmp/debug"; 
...
#ifdef	STREAMS
 	register int		argc2;		/* mu1	*/
 	register char		**argv2;	/* mu1	*/
 	register strlisttab_t	*strlistptr;	/* mu1	*/
 	register int		strforce;	/* mu2	*/
#endif	/* STREAMS */
...
/* timeout argument. */
				...
				else error1("uugetty: timeout argument invalid. \"%s\"\n\r", *argv);
			}
			break;
...
/* set, it will parse the entire file, printing out the results. */
			...
			exit(0);
#ifdef	STREAMS
 		case 's':	/* processed later - mu1	*/
 		case 'P':	/* processed later - mu1	*/
 		case 'S':	/* processed later - mu1	*/
 		case 'F':	/* processed later - mu1	*/
 		case 'f':	/* processed later - mu1	*/
#endif	/* STREAMS */
		default:
			break;
...
/* "init" from working too hard. */
	if(argc < 1) {
 		error1("uugetty: no terminal line specified.\n\r");
		sleep(20);
		exit(1);
	} else line = *argv;
...
#ifdef	STREAMS
#ifdef	DEBUG
error1("uugetty: argc=%d *argv=line='%s'\n\r",argc,*argv);
#endif	/* DEBUG */
 	/*
 	 *	mu0, mu1
 	 *	Any arguments after '-P' are assumed to be names of
 	 *	stream modules to be pushed in the order supplied.
 	 *	Save arg pointers for future, they are pointing to
 	 *	the last processed arg set.
 	 */
 	argc2 = argc;
 	argv2 = argv;
#endif	/* STREAMS */
...
/* "/etc/gettydefs" file.  If none was provided, take the first entry */
/* of the "/etc/gettydefs" file as the initial settings. */
	if(--argc > 0 ) {
#ifdef	STREAMS
 		if (**argv == '-') {	/* streams argument	mu1	*/
 			argc = argc2;	/* restore	*/
 			argv = argv2;	/* restore	*/
 			speedef = find_def(NULL);	/* use default	*/
 		} else	{
#endif	/* STREAMS */
		if((speedef = find_def(*++argv)) == NULL) {
			error1("uugetty: unable to find %s in \"%s\".\n\r",
			    *argv,GETTY_DEFS);
...
#ifdef	STREAMS
#ifdef	DEBUG
error1("uugetty: argc=%d *argv=speed='%s'\n\r",argc,*argv);
#endif	/* DEBUG */
			argc2 = argc;
			argv2 = argv;
		}
...
/* If a terminal type was supplied, try to find it in list. */
	if(--argc > 0) {
		...
		if (**argv == '-') {	/* streams argument	mu1	*/
 			argc = argc2;	/* restore	*/
 			argv = argv2;	/* restore	*/
 			termtype = TERM_NONE;	/* use default		*/
 		} else	{
#endif	/* STREAMS */
		if((answer = search(*++argv,terminals)) == NULL) {
 			error1("uugetty: %s is an undefined terminal type.\n\r",
			    *argv);
			termtype = TERM_NONE;
		} else termtype = answer->s_value;
#ifdef	STREAMS
#ifdef	DEBUG
error1("uugetty: argc=%d *argv=term='%s'\n\r",argc,*argv);
#endif	/* DEBUG */
 			argc2 = argc;
 			argv2 = argv;
		}
	...
	if(--argc > 0) {
		++argv;
 		...
 		if (**argv == '-') {	/* streams argument	mu1	*/
 			argc = argc2;	/* restore	*/
 			argv = argv2;	/* restore	*/
 			lined = LDISC0;	/* use default	*/
 		} else	{
#endif	/* STREAMS */
		if((answer = search(*++argv,linedisc)) == NULL) {
		error1("uugetty: %s is an undefined line discipline.\n\r",
			    *argv);
			lined = LDISC0;
		} else lined = answer->s_value;
#ifdef	STREAMS
#ifdef	DEBUG
error1("uugetty: argc=%d *argv=disc='%s'\n\r",argc,*argv);
#endif	/* DEBUG */
			argc2 = argc;
			argv2 = argv;
		}
...
#ifdef STREAMS
	/*	mu1
	 *	All remaining arguments pertain to streams.
	 *	Build a list of module names to be pushed in openline().
	 */
	argc--;
	argv++;
	argv2 = argv;
	strlistptr = &strlisttab[0];
	strforce = 0;	/* mu2 */
#ifdef	DEBUG
error1("uugetty: check stream args: argc=%d *argv='%s'\n\r",argc,argc > 0 ? *argv : "NONE");
#endif	/* DEBUG */
	while (argc > 0) {
		if (**argv == '-') {
			(*argv)++;		/* skip the dash	*/
			while (**argv) {
				...
				case 'P':	/* single letter option	*/
#ifdef	DEBUG
error1("uugetty: argc=%d *argv='%s': found -P\n\r",argc,*argv);
#endif	/* DEBUG */
					break;

				case 'F':	/* single letter option	*/
#ifdef	DEBUG
error1("uugetty: argc=%d *argv='%s': found -F\n\r",argc,*argv);
#endif	/* DEBUG */
					strforce = STRFLG_FORCE;
					break;
...
#ifdef	DEBUG
error1("uugetty: argc=%d *argv='%s': found -S\n\r",argc,*argv);
#endif	/* DEBUG */
					strsaveall = 1;
					break;
...
#ifdef	DEBUG
error1("uugetty: argc=%d *argv='%s': found -s: ",argc,*argv);
#endif	/* DEBUG */
					strsaveall = 0;
					if (**argv && isascii(**argv)) {
						strlistptr->name = *argv;
#ifdef	DEBUG
error1("name='%s'",*argv);
#endif	/* DEBUG */
						strlistptr->flags =
							STRFLG_SAVE | strforce;
						strlistptr++;
 						while (**argv)
 							(*argv)++;
 					} else	{
 						if (argc > 1) {
 							argv2++;
 							argc--;
 							strlistptr->name=*argv2;
#ifdef	DEBUG
error1("name2='%s'",*argv2);
#endif	/* DEBUG */
							strlistptr->flags =
								STRFLG_SAVE |
								strforce;
...
error1("\n\r");
 				...
 				default:
#ifdef	DEBUG
error1("uugetty: argc=%d *argv='%s': unknown option\n\r",argc,*argv);
#endif	/* DEBUG */
 					break;
 				}
 			}
...
#ifdef	DEBUG
error1("uugetty: argc=%d *argv='%s': name='%s'\n\r",argc,*argv,*argv);
#endif	/* DEBUG */
			strlistptr->name  = *argv;
 			strlistptr->flags = strforce;
 			strlistptr++;
 		}
 		argv++;
 		argv2 = argv;
 	...
 	strlistptr->name = NULL;
 	strlistptr->flags = 0;
#ifdef	DEBUG
strlistptr = &strlisttab[0];
error1("uugetty: modules found: ");
while (strlistptr->name != NULL) {
 	error1("'%s' ",strlistptr->name);
 	strlistptr++;
}
if (strlistptr == &strlisttab[0])
	error1("NONE");
error1("\n\r");
#endif	/* DEBUG */
#endif	/* STREAMS */
...
#ifdef DEBUG
	error1( "Leaving account(%s).\n\r", line );
#endif /* DEBUG */

/* Attempt to open standard input, output, and error1 on specified */
/* line. */
	chdir("/dev");
...
#ifdef DEBUG
	error1( "Trying to open line, %s.\n\r", line );
#endif /* DEBUG */

/* setup initial timer values now, instead of after fopen completes  kd0*/
	if ((fd=open(line,O_RDWR | O_NDELAY)) == -1) {
		error1("getty: cannot open \"%s\". errno: %d\n",line,errno);
		sleep(20);
		exit(1);
	}

#ifdef DEBUG
	error1( "%s successfully opened, file descriptor is %d.\n\r", line, fd );
#endif /* DEBUG */

	ioctl(fd,GCTIO_SET,&i_gctio); 

	/* BEFORE STREAMS uugetty issued an 'fopen' and hung until the 'fopen'
	 * completed.  Completion happens when a DSR/DCD are asserted which
	 * happens when either a remote system dials in or a local process
	 * (cu or ct) dials out  successfully.   When a local process dials
	 * out an open is issued on the selected port. If uugetty is running
	 * on the device and the device is NOT a streams device two pending
	 * opens occur.  If the local process gets a connect, uugetty's open
	 * completes and uugetty goes away......
	 *
	 * AFTER STREAMS two pending opens are not allowed on the streamhead.
	 * If uugetty is running on a device, and a local process (cu or ct)
	 * the local process will not be able to get at the device even if the
	 * line associated with the device is not busy.
	 *
	 * wait_for_connect_connect(fd) - fixes uugettys two pending open on the
	 * stream head by letting the above 'open()' NDELAY by used to open
	 * the device.  wait_for_connect_notify will poll for a message
	 * indicating that DSR/DCD has occured indicating that an open has
	 * really completed.
	 */
#ifdef DEBUG
	error1( "Entering wait_for_connect_notify(%d).\n\r", fd );
#endif /* DEBUG */

	wait_for_connect_notify(fd);

#ifdef DEBUG
	error1( "Leaving wait_for_connect_notify(%d).\n\r", fd );
#endif /* DEBUG */

	if ( (fdopen(fd, "r+")) == NULL)
	{ /* this opens stdin */
		error1("uugetty: cannot open \"%s\". errno: %d\n",line,errno);
		sleep(20);
		exit(1);
	}

	/* Get the flags associated with file descriptor 'fd' */
	if (fcntl(fd, F_GETFL, fcntlargs) < 0) {
		error1("uugetty: fcntl(..,F_GETFL,..) failed!\n");
		exit(1);
	}

	/* Turn off the O_NDELAY bit */
	fcntlargs &= ~O_NDELAY;

	/* Set the flags associated with file descriptor 'fd' */
	if (fcntl(fd, F_SETFL, fcntlargs) < 0) {
		error1("uugetty: fcntl(..,F_GETFL,..) failed!\n");
		exit(1);
	}
 	 ...
 	 */
#ifdef	DEBUG
error1("uugetty: check for lock file\n\r");
#endif	/* DEBUG */
error1("uugetty: check for lock file\n\r");
 	 ...
 	 */
#ifdef	DEBUG
error1("uugetty: remove created uugetty lockfile\n\r");
#endif	/* DEBUG */
 	delock(line);
...
#ifdef DEBUG
	error1( "Entering setupline(%s, %d, %d).\n\r", speedef->g_id, termtype, lined );
#endif /* DEBUG */

		setupline(speedef,termtype,lined); 

#ifdef DEBUG
	error1( "Leaving setupline(%s, %d, %d).\n\r", speedef->g_id, termtype, lined );
#endif /* DEBUG */
		...
		{
			error1("uugetty: read(0,buffer) < 0, errno: %d\n\r", errno);
			(void) fclose(stdin);
			sleep(10);
			exit(0);
		}
...
error1("uugetty: check for lock file\n\r");
	...
	if (mlock(line)) { /*  There is a lock file already */
#ifdef	DEBUG
error1("uugetty: there is a lock file already\n\r");
#endif	/* DEBUG */
	    /* some process is using the line for output */
	    (void) fclose(stdin);
	    (void) sprintf(lckname, "%s.%s", LOCKPRE, line);
...
#ifdef	DEBUG
error1("uugetty: openline(%d,%s,%d,%d,%d).\n\r", line, speedef->g_id, termtype, lined, hangup);
#endif	/* DEBUG */
...
#ifdef	DEBUG
error1("uugetty: loop until user is successful\n\r");
#endif	/* DEBUG */
...
#ifdef	DEBUG
error1("uugetty: print the issue file\n\r");
#endif	/* DEBUG */
...
#ifdef	DEBUG
error1("uugetty: print the login message\n\r");
#endif	/* DEBUG */
...
/* Get the user's typed response and respond appropriately. */
		...
		case GOODNAME:
#ifdef	DEBUG
error1("uugetty: getname GOODNAME\n\r");
#endif	/* DEBUG */
			if (timeout) alarm(0);
...
/* on when the terminal type was set. */
			...
			fioctl(stdin,GCTIO_SET,&f_gctio);	/*kd0*/
...
#ifndef	DEBUG1
#ifdef	DEBUG
error1("uugetty: execv /bin/login\n\r");
#endif	/* DEBUG */
...
/* If the speed supplied was bad, try the next speed in the list. */
		case BADSPEED:
#ifdef	DEBUG
error1("uugetty: getname BADSPEED\n\r");
#endif	/* DEBUG */
...
/* won't overwrite it in the process of looking for new entry. */
			...
			if ((speedef = find_def(newspeed)) == NULL) {
				error1("uugetty: pointer to next speed in entry %s is bad.\n\r",
				oldspeed);

/* In case of error1, go back to the original entry. */
				if((speedef = find_def(oldspeed)) == NULL) {

/* If the old entry has disappeared, then quit and let next "getty" try. */
					error1("uugetty: unable to find %s again.\n\r",
						oldspeed);
					exit(1);
				}
			}
...
pushmodules()
{
 	...
 	register strlisttab_t	*strlistptr;	/* mu1	*/
 	register int		strfind, stronesuccess;	/* mu2 */
	extern int errno;
	 ...
	 */
	strlistptr = &strlisttab[0];
	if (strlistptr->name == NULL) {
/*mbm0 --everything before the 'else' pushes the default module */
		if ((strfind = fioctl(stdin,I_FIND,defstrname)) == 0) {
			fioctl(stdin,LDGETT,&termcb);
			fioctl(stdin,TCGETA,&termio);
			if (fioctl(stdin,I_PUSH,defstrname) < 0) {
		error("getty: can't push stream module '%s' (errno=%d)\n\r",
					defstrname,errno);
			} else	{
				fioctl(stdin,LDSETT,&termcb);
				fioctl(stdin,TCSETAF,&termio);
				/*
				 *	Pause very, very briefly while
				 *	terminal settles.
				 */
				for (timer = 0; ++timer != 0; );
			}
 		...
 		while (strlistptr->name != NULL) {
			strfind = fioctl(stdin,I_FIND,strlistptr->name);
 			if (strfind < 0) {
 	error1("uugetty: stream module '%s' not supported (errno=%d)\n\r",
 					strlistptr->name,errno);
 				strlistptr++;
 				continue;
 			}
 			if (strfind > 0) {
 		error1("uugetty: stream module '%s' is already pushed\n\r",
 					strlistptr->name);
 				if (!(strlistptr->flags & STRFLG_FORCE)) {
 					strlistptr++;
 					continue;
 				}
 		error1("uugetty: force another push of stream module '%s'\n\r",
 					strlistptr->name);
 			}
 			if (strlistptr->flags & STRFLG_SAVE) {
 				fioctl(stdin,LDGETT,&termcb);
 				fioctl(stdin,TCGETA,&termio);
 				strsaveall = 0;	/* just in case	*/
 			}
#ifdef	DEBUG
error1("uugetty: pushmodules %s \n\r",strlistptr->name);
#endif	/* DEBUG */
 			if (fioctl(stdin,I_PUSH,strlistptr->name) < 0) {
 		error1("uugetty: can't push stream module '%s' (errno=%d)\n\r",
 					strlistptr->name,errno);
 				strlistptr++;
 				continue;
 			}
...
#ifdef	DEBUG
error1("uugetty: setupline enter\n\r");
#endif	/* DEBUG */
	termcb.st_flgs = 0;
...
/* Pause briefly while terminal settles. */
	for(timer=0; ++timer != 0;);
#ifdef	DEBUG
error1("uugetty: setupline exits\n\r");
#endif	/* DEBUG */
}
...
/* If it isn't possible to read line, exit. */
		...
		rawc = getc(stdin);
#ifdef	DEBUG2
error1("uugetty: getname 3 rawc=%d\n\r", rawc);
#endif	/* DEBUG */
		if (rawc  == EOF) 
		{
#ifdef	DEBUG2
error1("uugetty: impossible to read line, exit \n\r");
#endif	/* DEBUG */
...
/*	the Gdef structure filled.  Errors in parsing generate error1	*/
...
/* Initialize gctio's */
		i_gctio.gt_tdcd = 0;	/*kd0*/
		i_gctio.gt_tact = 0;	/*kd0*/
		f_gctio.gt_tdcd = 0;	/*kd0*/
		f_gctio.gt_tact = 0;	/*kd0*/
...
/* Extract the "g_iflags" */
			case IFLAGS:
				if ((ptr = fields(ptr,&gptr->g_iflags,&i_gctio)) == NULL) { /*kd0*/
					field = state;
					state = FAILURE;
				} else {
...
/* Extract the "g_fflags". */
			case FFLAGS:
				if ((ptr = fields(ptr,&gptr->g_fflags,&f_gctio)) == NULL) { /*kd0*/
					field = state;
					state = FAILURE;
				} else {
...
/* In check mode print out the results of the parsing. */
				...
				if(i_gctio.gt_tdcd || i_gctio.gt_tact) /*kd0*/
				fprintf(stdout,"  tdcd - %d  tact- %d\n",
					i_gctio.gt_tdcd,i_gctio.gt_tact);
				else fprintf(stdout,"\n");
				...
				if(f_gctio.gt_tdcd || f_gctio.gt_tact) /*kd0*/
				fprintf(stdout,"  tdcd - %d  tact- %d\n",
					f_gctio.gt_tdcd,f_gctio.gt_tact);
				else fprintf(stdout,"\n");
...
char *fields(ptr,termio,gctio) /*kd0*/
register char *ptr;
struct termio *termio;
struct gctio *gctio; /*kd0*/
{
...
/* that are needed for SANE tty behavior. */
			...
			} else if(strcmp(word,"TDCD") == 0) { 	/*kd0*/
				ptr += size; 			/*kd0*/
				word = getword(ptr,&size); 	/*kd0*/
				value = atoi(word); 		/*kd0*/
				if(value < 1 || value > 18000) 	/*kd0*/
					goto eval;		/*kd0*/
				gctio->gt_tdcd = value;		/*kd0*/
				gctio->gt_tdcd_sig = SIGHUP;
			} else if(strcmp(word,"TACT") == 0) { 	/*kd0*/
				ptr += size; 			/*kd0*/
				word = getword(ptr,&size); 	/*kd0*/
				value = atoi(word); 		/*kd0*/
				if(value < 1 || value > 18000) 	/*kd0*/
					goto eval;		/*kd0*/
				gctio->gt_tact = value;		/*kd0*/
				gctio->gt_tact_sig = SIGHUP;
...
/* wait_for_connect_notify(fd) - Algorithm:
 *
 *             ioctl to turn on connect notify;
 *	       poll for the notify;
 *             ioctl to turn off connect notify;
 *	       getmsg to get the message;
 *	       while ( poll for spurious messages )
 *			getmsg to get the spurious messages;
 */
void
wait_for_connect_notify(fd)
int     fd;
{
        struct pollfd   fds[1];         /* Data passed to poll. */
        int             retval = 0;     /* Returned control message */
        struct strbuf   ctl;            /* Streams buffer control pointer */
        struct strbuf   data;           /* Streams buffer data pointer */
        int             flags = 0;      /* Flags passed to getmsg */
        int             tc_arg = 0;     /* Flags passed to ioctl */

        tc_arg = TC_CONNOTIFY_ON;
        if (ioctl(fd, TCCON_ALERT, tc_arg) < 0) {
		error1("uugetty:ioctl(fd, TCCON_ALERT, &tc_arg) failed\n\r");
		error1("uugetty: errno: %d\n\r",errno);
		exit(1);
	}

        fds[0].fd      = fd;
        fds[0].events  = POLLPRI; /* Event: priority message on stream head. */
        fds[0].revents = 0;

        /* Poll will block until we get a high priority message on the
         * stream head.  The message indicates DSR/DCD (a connect).
         */
#ifdef DEBUG
	error1( "Polling on %d.\n\r", fds[0].fd );
#endif /* DEBUG */
        ...
        if (poll(fds, 1, -1) == -1) {
		error1("uugetty:poll failed! errno: %d\n\r", errno);

		tc_arg = TC_CONNOTIFY_OFF;	/* Connect notify OFF */
		if (ioctl(fd, TCCON_ALERT, tc_arg) < 0) {
			error1("uugetty:1 fd:%d, TCCON_ALERT:%x, tc_arg:%d\n\r",
				fd,TCCON_ALERT,tc_arg);
			error1("uugetty:1cleanup ioctl failed\n\r");
			error1("uugetty:1 errno: %d\n\r",errno);
			exit(1);
		}
		return;
	}
...
#ifdef DEBUG
	error1( "Got a message continuing.\n\r" );
#endif /* DEBUG */

        tc_arg = TC_CONNOTIFY_OFF;
        if (ioctl(fd, TCCON_ALERT, tc_arg) < 0) {       /* Connect notify OFF */
		error1("uugetty:2ioctl(fd, TCCON_ALERT, tc_arg) failed\n\r");
		error1("uugetty:2fd-> %d, TCCON_ALERT-> %x, tc_arg-> %d\n\r",
			fd,TCCON_ALERT,tc_arg);
		error1("uugetty:2 errno: %d\n\r",errno);
		exit(1);
	}

	/* Initialize data and control structures. */
        data.maxlen = 0;
        data.len    = 0;
        data.buf    = (char *)NULL;
...
#ifdef DEBUG
	error1( "Now polling on %d for spurious messages.\n\r", fds[0].fd );
#endif /* DEBUG */
        ...
        while (poll(fds, 1, 50) > 0) {
                ...
                getmsg(fd, &ctl, &data, &flags);
#ifdef DEBUGIT
	error1( "Got a second message %s!!", data );
#endif
        }
}
