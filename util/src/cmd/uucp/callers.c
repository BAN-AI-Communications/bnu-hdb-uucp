...
/*							*/
/*	@(#) callers.c: version 25.1 created on 12/2/91 at 18:13:22	*/
/*							*/
/*	Copyright (c) 1990 by Arix Corporation		*/
/*	All Rights Reserved				*/
/*							*/
#ident	"@(#)callers.c	25.1	12/2/91 Copyright (c) 1990 by Arix Corporation"
/*							*/
...
/*	ATT:#ident	"uucp:callers.c	2.21"			*/

#ident	"@(#)uucp:callers.c	25.1"
#define STREAMS 1
 ...
 */
#ifdef STREAMS
processdev(flds, dev, av)
register char *flds[], *dev[], *av[];
#else
processdev(flds, dev)
...
#endif
{
	...
	char	complete_device [512];

	sdev = dev;

	/* -  This routine verifies that the user actually
	   	  has access to the device they are attempting
		  to use */

/*SECURE -This checks the dev_auth file if approate*/

	sprintf (complete_device, "/dev/%s", dev[D_LINE] );
	if ( ! uid_devinq ( getuid (), complete_device ) ){

		DEBUG (5, "Authorization check failed on device %s\n",
		       complete_device);
...
#ifdef STREAMS
#ifdef DEBUG1
		VERBOSE("processdev: ca->CA_caller\n", "");
#endif
			if ((dcf = (*(ca->CA_caller))(flds, dev,av)) < 0)
#else
	...
	if (dcf == -1) {
		...
		*/
		/* lets always open with O_NDELAY */
		dcf = open(dcname, (O_RDWR | O_NDELAY) );
		(void) alarm(0);
		if (dcf < 0) {
...
#ifdef	DEBUG1
	arg=*av;
VERBOSE("processdev: av=%s\n",arg, "");
VERBOSE("processdev: pushmodules\n", "");
#endif	/* DEBUG */
		...
		*/
		if(fcntl(dcf, F_SETFL,(fcntl(dcf,F_GETFL,0) & ~O_NDELAY)) < 0) {
			DEBUG( 7, "clear O_NDELAY failed, errno %d\n", errno);
				(void)close(dcf);
				Uerror = SS_DEVICE_FAILED;
				goto bad;
...
#ifdef	DEBUG1
	arg=*av;
VERBOSE("sytcall: av=%s\n",arg, "");
VERBOSE("sytcall: pushmodules\n", "");
#endif	/* DEBUG */
	pushmodules(dcr,av);
#endif
...
#ifdef	DEBUG1
	arg=*av;
VERBOSE("dial801: av=%s\n",arg);
VERBOSE("dial801: pushmodules\n", "");
#endif	/* DEBUG */
		pushmodules(dcf,av);
#endif
